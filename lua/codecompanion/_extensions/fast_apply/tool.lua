...
local function apply_changes_async(action, cb)
	...
	local tool_opts = (
		...
		and config.interactions.chat.tools.fast_apply
		and config.interactions.chat.tools.fast_apply.opts
	) or {}

	-- Determine adapter source: tool opts adapter -> global chat adapter -> fallback
	local adapter_source = tool_opts.adapter
		or (config.interactions and config.interactions.chat and config.interactions.chat.adapter)
